...
export class Anim {
    ...
    renderFrame(i, dir) {
        if (this.moving) {
            ...
            ctx.drawImage(this.img, this.currMove[i].x0, this.currMove[i].y0 + this.skin * 16, canSize, canSize, 0, 0, canSize, canSize);
        }
    }
    goTo(x, y) {
...
export class AnimBaloon extends Anim {
    constructor(img, ob, id, pos, currDir) {
        ...
        if (this.repeat) {
            ...
            if (obj[this.currDir]) {
                ...
                if (this.skin == 3) { //pomarancz
                    ...
                    this.currDir = dirs[Math.floor(Math.random() * 4)];
                return;
            }
            if (Math.random() >= 0.8 || (Math.random() >= 0.6 && this.skin == 3))
                this.currDir = dirs[Math.floor(Math.random() * 4)];
        }
    }
}
export class AnimPlayer extends Anim {
    constructor(img, ob, pos, currDir) {
